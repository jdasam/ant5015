...
def make_harmonic_series(fundamental_freq, num_harmonics, amp_weights, duration, sr):
  '''
  배음(harmonics)을 합성하여 복합음(complex tone)을 생성한다.

  Inputs:
    fundamental_freq (float): 기본음의 주파수 (Hz)
    num_harmonics (int): 배음의 수 (1배음인 기본음 포함)
    amp_weights (list or torch.Tensor): 각 배음의 진폭 가중치, 길이: num_harmonics
                                        amp_weights[0]은 1배음(기본음), amp_weights[1]은 2배음, ...
    duration (float): 소리의 길이 (초)
    sr (int): sampling rate

  Output:
    복합음 (torch.Tensor), shape: (num_samples,)
    - make_sine_wave()를 활용하여 각 배음의 사인파를 합산하여 반환
  '''
  assert len(amp_weights) == num_harmonics
  # TODO: 주어진 입출력 형식과 내용을 만족하는 함수를 완성하시오
  # Hint: make_sine_wave()를 반복 호출하여 각 배음을 합산한다
  #       n번째 배음의 주파수 = fundamental_freq * n  (n은 1부터 시작)

  return


def make_square_wave(fundamental_freq, num_harmonics, duration, sr):
  '''
  사각파(Square Wave)를 배음 합성으로 생성한다.
  사각파 = 홀수 배음만 포함 (1, 3, 5, ...), n번째 배음의 진폭 = 1/n

  Inputs:
    fundamental_freq (float): 기본음의 주파수 (Hz)
    num_harmonics (int): 포함할 배음의 최대 수 (이 수까지의 홀수 배음만 사용)
    duration (float): 소리의 길이 (초)
    sr (int): sampling rate

  Output:
    사각파 (torch.Tensor), shape: (num_samples,)
  '''
  # TODO: make_harmonic_series()를 활용하여 사각파를 생성하시오
  # Hint: 홀수 배음(1, 3, 5, ...)의 가중치는 1/n, 짝수 배음의 가중치는 0
  #       예: num_harmonics=5 이면 amp_weights = [1/1, 0, 1/3, 0, 1/5]

  return


def make_sawtooth_wave(fundamental_freq, num_harmonics, duration, sr):
  '''
  톱니파(Sawtooth Wave)를 배음 합성으로 생성한다.
  톱니파 = 모든 배음 포함 (1, 2, 3, ...), n번째 배음의 진폭 = 1/n

  Inputs:
    fundamental_freq (float): 기본음의 주파수 (Hz)
    num_harmonics (int): 포함할 배음의 최대 수
    duration (float): 소리의 길이 (초)
    sr (int): sampling rate

  Output:
    톱니파 (torch.Tensor), shape: (num_samples,)
  '''
  # TODO: make_harmonic_series()를 활용하여 톱니파를 생성하시오
  # Hint: 모든 배음 포함, n번째 배음의 진폭 = 1/n
  #       예: num_harmonics=4 이면 amp_weights = [1/1, 1/2, 1/3, 1/4]

  return


def apply_adsr(audio, attack, decay, sustain_level, release, sr):
  '''
  오디오 샘플에 ADSR 엔벨로프를 적용한다.

  Inputs:
    audio (torch.Tensor): 1D 오디오 텐서, shape: (num_samples,)
    attack (float): Attack 시간 (초) - 진폭 0 → 1 (선형 증가)
    decay (float): Decay 시간 (초) - 진폭 1 → sustain_level (선형 감소)
    sustain_level (float): Sustain 레벨 (0~1)
    release (float): Release 시간 (초) - 진폭 sustain_level → 0 (선형 감소, 오디오 끝)
    sr (int): Sampling rate

  Output:
    ADSR 엔벨로프가 적용된 오디오 (torch.Tensor), shape: (num_samples,)

  주의: attack + decay + release 샘플 수가 전체 샘플 수를 초과하지 않는다고 가정한다.
  '''
  ...
  assert sustain_samples >= 0, "attack + decay + release가 전체 오디오 길이를 초과합니다."

  # TODO: 주어진 입출력 형식과 내용을 만족하는 함수를 완성하시오
  # 각 구간의 엔벨로프 텐서를 만든 뒤 torch.cat()으로 연결하고 audio에 곱한다
  # - Attack:  torch.linspace(0, 1, attack_samples)
  # - Decay:   torch.linspace(1, sustain_level, decay_samples)
  # - Sustain: torch.full((sustain_samples,), sustain_level)
  # - Release: torch.linspace(sustain_level, 0, release_samples)
